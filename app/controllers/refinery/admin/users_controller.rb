module Refinery
  module Admin
    class UsersController < Refinery::AdminController
      ...
      def create
        ...
        if @user.save
          ...
          unless current_refinery_user.has_role?(:superuser) and Refinery::Authentication.superuser_can_assign_roles
            ...
            @user.roles = @selected_role_names.collect { |r| Spree::Role[r.downcase.to_sym] }
          end
      ...
      def update
        ...
        if current_refinery_user.id == @user.id and (params[:user][:plugins].exclude?("refinery_users") || @selected_role_names.map(&:downcase).exclude?("refinery"))
          flash.now[:error] = t('cannot_remove_user_plugin_from_current_user', :scope => 'refinery.refinery_admin.users.update')
          render :edit
        else
          ...
          logger.debug "@selected_role_names == #{@selected_role_names}"
          logger.debug "Spree::Role[:refinery] == #{Spree::Role[:refinery]}"
          logger.debug "@user.roles = #{@selected_role_names.collect { |r| Spree::Role[r.downcase.to_sym] }}"
          @user.roles = @selected_role_names.collect { |r| Spree::Role[r.downcase.to_sym] }
    ...
    def load_available_plugins_and_roles
      ...
      @available_roles = Spree::Role.all
    end
